...
import { DialogActions, DialogButtonsList } from "@/components/ui/lexcial/dialog";
...
export function InsertImageUploadedDialogBody({ onClick }: { onClick: (payload: InsertImagePayload) => void }) {
  ...
  const loadImage = async (files: FileList | null) => {
    ...
    reader.onload = async function () {
      if (typeof reader.result === "string") {
        ...
        const imageFile = Buffer.from(dataURL.replace(/^data:image\/\w+;base64,/, ""), "base64");
  ...
  return (
    <>
      ...
      <TextInput label="Alt Text" placeholder="Descriptive alternative text" onChange={setAltText} value={altText} data-test-id="image-modal-alt-text-input" />
      <DialogActions>
        <Button data-test-id="image-modal-alt-text-input" disabled={isDisabled} onClick={() => onClick({ altText, src })}>
          Confirm
        </Button>
      </DialogActions>
    </>
...
export function InsertImageUriDialogBody({ onClick }: { onClick: (payload: InsertImagePayload) => void }) {
  ...
  return (
    <>
      <TextInput label="Image URL" placeholder="i.e. https://source.unsplash.com/random" onChange={setSrc} value={src} data-test-id="image-modal-url-input" />
      <TextInput label="Alt Text" placeholder="Random unsplash image" onChange={setAltText} value={altText} data-test-id="image-modal-alt-text-input" />
      <DialogActions>
        <Button disabled={isDisalbed} onClick={() => onClick({ altText, src })} data-test-id="image-modal-confirm-btn">
          Confirm
        </Button>
      </DialogActions>
    </>
    ...
    <>
      {!mode && (
        <DialogButtonsList>
              ...
              onClick(
                hasModifier.current
                  ? {
                      ...
                      src: "/img/amazing-spiderman-main.jpg",
                    }
                  : {
          ...
          <Button data-test-id="image-modal-option-url" className="mb-2" onClick={() => setMode("url")}>
            URL
          </Button>
