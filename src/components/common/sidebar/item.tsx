  ...
  return (
    <button
      ...
      className={cn(
        ...
        isActive && "text-orange-500 bg-orange-50 hover:bg-orange-50 hover:text-orange-500 "
      )}
    >
      <div className="flex items-center gap-x-2 py-4">
        <Icon className={cn("text-slate-400 h-4 w-4 mr-2", isActive && "text-orange-500")} />
        {label}
      </div>
    </button>
  );
...
export const MobileSidebarItem = ({ icon: Icon, href, label }: MobileSidebarItemProps) => {
  const pathname = usePathname();
  const router = useRouter();
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            <div className="flex items-center gap-x-2 py-4">
              <Icon size={22} className={cn("text-slate-400", isActive && "text-orange-500")} />
            </div>
          </button>
        </TooltipTrigger>
        <TooltipContent side="right" className="rounded-md text-sm text-slate-500">
          {label}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
